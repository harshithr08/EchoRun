...
typedef struct {
    ...
    //CHANGE
    int            should_quit;    // scripted REPL quit requested a clean replay shutdown
    FILE          *repl_input;     // optional script stream, falls back to stdin
    TelemetryWriter *telemetry;    // optional additive telemetry stream
    //CHANGE
} ReplaySession;
